 ...
 *   폼 필드 sidoNm·sggNm·pageIndex, 헤더 Referer 필요. 응답 `result[]` 에
 *   PLC_NM(지점명) · MANAGEMENT(일반지역|중점관리지역|위험지역) · PLC_TYPE(계곡|하천|…) · ADRES ·
...
import { loadSeedValleys, type SeedValley } from './valleys.mts';
...
export function safetyRowsOf(main: Candidate, hazards: readonly Candidate[]): SafetyRows {
  ...
  const banned = place.MANAGEMENT === '위험지역';
  let riskNote = `물놀이관리지역(${place.MANAGEMENT.replace('지역', '')}) ${place.PLC_NM} — 최대 수심 ${place.WATER_DEEP} m·평균 ${place.WATER_AVG} m`;
  if (banned) riskNote += ', 입수 금지';
  const hazard = hazards[0];
  if (!banned && hazard !== undefined) {
    riskNote += `. 인근 ${hazard.place.PLC_NM}는 위험지역(수심 ${hazard.place.WATER_DEEP} m)`;
  }
  const how =
  ...
  const note =
    ...
    (hazard === undefined
      ? ''
      : `; 위험지역 OBJT_ID ${hazard.place.OBJT_ID} ${hazard.distanceM} m`);
  return {
    swimBanned: banned ? 'true' : 'false',
    riskNote: riskNote.slice(0, 80),
    depth: depthOf(place),
    note,
  };
}
...
function report(
  ...
  entries.forEach(({ valley, candidates, main }, index) => {
    const others = candidates
      ...
      .map(
        (candidate) =>
          `${candidate.place.PLC_NM}(${candidate.place.MANAGEMENT.replace('지역', '')}, ${candidate.distanceM})`,
      )
      .join(' · ');
    if (main === undefined) {
      lines.push(
...
async function main(): Promise<void> {
  ...
  for (const valley of valleys) {
    ...
    const hazards = accepted.filter(
      (candidate) => candidate !== main && candidate.place.MANAGEMENT === '위험지역',
    );
